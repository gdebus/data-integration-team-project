...
print(f"[FUSED PATH] {FUSION_PATH}")
print(f"[EVAL SET PATH] {EVAL_SET_PATH}")
print(f"[ROWS] fused={len(fused)}, fused_eval={len(fused_eval)}, gold={len(fusion_eval_set)}")


# ================================
# 2. DROP NON-1:1 EVAL COLUMNS / KEEP ONLY DIRECT OVERLAP
# ================================
...
meaningful_shared_columns = [
    ...
    if _is_meaningful_column(fused_eval, c) and _is_meaningful_column(fusion_eval_set, c)
]

# Parent/subcolumn safeguard: only direct overlaps survive by construction.
# Still exclude provenance/meta fields.
eval_columns = [
    c for c in meaningful_shared_columns
    if not c.startswith("_") and not c.endswith("_provenance")
]

# ================================
# 3. NORMALIZE LIST-LIKE COLUMNS TO GOLD REPRESENTATION
# ================================
...
list_eval_columns = [c for c in list_eval_columns if c in eval_columns]
...
if list_eval_columns:
    ...
    print(f"[LIST NORMALIZATION] {', '.join(list_eval_columns)}")


# ================================
# 4. NORMALIZE COUNTRY COLUMNS TO GOLD REPRESENTATION
# ================================
country_eval_columns = [c for c in eval_columns if "country" in c.lower()]
...
# ================================
# 5. COERCE NUMERIC-LIKE COLUMNS CONSISTENTLY
# ================================
numeric_eval_columns = [c for c in eval_columns if c in {"duration"}]
...
# ================================
strategy = DataFusionStrategy("music_validation_evaluation_strategy")
eval_funcs_summary = {}
...
for col in eval_columns:
    ...
    elif "date" in col.lower() or col.lower() in {"founded", "release-date"}:
        register_eval(col, year_only_match, "year_only_match")
    elif col in numeric_eval_columns:
        register_eval(col, numeric_tolerance_match, "numeric_tolerance_match", tolerance=5.0)
    elif col.lower() in {"id", "code", "isbn", "issn", "doi"}:
        register_eval(col, exact_match, "exact_match")
    elif col.lower().startswith("is_") or col.lower().startswith("has_") or col.lower() in {"explicit"}:
...
print(json.dumps(evaluation_results, indent=2, sort_keys=True))
