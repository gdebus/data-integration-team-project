...
def build_eval_view(fused_df, gold_df):
    ...
    If fused already contains eval_id, keep it as primary.
    """
    if "eval_id" in fused_df.columns:
        ...
        if "_id" in out.columns:
            ...
            out["_eval_cluster_id"] = out["eval_id"]
...
def _infer_country_output_format(gold_df, country_columns):
    ...
    official_name = sum(1 for v in values if any(tok in v.lower() for tok in ["republic", "kingdom", "states of", "federation"]))
...
def _coerce_numeric_consistent(df, col):
...
def _gold_has_meaningful_values(df, col):
    return col in df.columns and df[col].notna().any()


def _is_list_like_series(series):
    sample = series.dropna().head(100)
    if sample.empty:
        return False
    return sample.apply(lambda x: isinstance(x, (list, tuple, set))).mean() >= 0.3
...
# ================================
# 2. DROP NON-EVALUABLE / NON-1:1 COLUMNS
# ================================
shared_columns = set(fused_eval.columns) & set(fusion_test_set.columns)
shared_columns.discard("id")
shared_columns.discard("_id")
shared_columns.discard("eval_id")
shared_columns.discard("_eval_cluster_id")

# Drop provenance and internal columns
shared_columns = {c for c in shared_columns if not c.endswith("_provenance") and not c.startswith("_")}

# Enforce direct 1:1 matches only; if gold lacks exact column, do not evaluate it.
# This naturally prevents evaluating derived columns against absent parent columns.
meaningful_shared_columns = [
    c for c in sorted(shared_columns)
    if _gold_has_meaningful_values(fusion_test_set, c) and fused_eval[c].notna().any()
]

# ================================
# 3. NORMALIZE LIST-LIKE COLUMNS FOR EVALUATION
# ================================
candidate_list_columns = detect_list_like_columns(
    [fused_eval[["eval_id"] + [c for c in meaningful_shared_columns if c in fused_eval.columns]].copy(),
     fusion_test_set[["id"] + [c for c in meaningful_shared_columns if c in fusion_test_set.columns]].copy()],
    exclude_columns={"id", "_id", "eval_id", "_eval_cluster_id", "_fusion_confidence"},
)
list_eval_columns = [c for c in candidate_list_columns if c in meaningful_shared_columns]
...
# ================================
# 4. NORMALIZE COUNTRY COLUMNS FOR EVALUATION
# ================================
country_eval_columns = [c for c in meaningful_shared_columns if "country" in str(c).lower()]
...
# ================================
numeric_columns = []
for col in meaningful_shared_columns:
    col_l = col.lower()
    if any(tok in col_l for tok in ["duration", "year", "count", "rating", "score"]):
        numeric_columns.append(col)

for col in numeric_columns:
    _coerce_numeric_consistent(fused_eval, col)
    _coerce_numeric_consistent(fusion_test_set, col)
...
for col in meaningful_shared_columns:
    ...
    if col in list_eval_columns or _is_list_like_series(fusion_test_set[col]) or _is_list_like_series(fused_eval[col]):
        register_eval(col, set_equality_match, "set_equality_match")
    elif "date" in col_l or "released" in col_l or "founded" in col_l:
        register_eval(col, year_only_match, "year_only_match")
    elif any(tok in col_l for tok in ["duration", "revenue", "price", "amount", "rating", "score", "latitude", "longitude", "count", "year"]):
        tolerance = 10.0 if "duration" in col_l else 0.01
        register_eval(col, numeric_tolerance_match, "numeric_tolerance_match", tolerance=tolerance)
    elif any(tok in col_l for tok in ["id", "code"]) and col != "artist" and col != "name":
        register_eval(col, exact_match, "exact_match")
    elif fusion_test_set[col].dropna().astype(str).str.lower().isin(["true", "false"]).mean() > 0.8:
    ...
    elif any(tok in col_l for tok in ["name", "artist", "label", "genre", "title"]):
