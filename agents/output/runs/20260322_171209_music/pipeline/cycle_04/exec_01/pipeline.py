...
discogs = load_csv(
    "output/runs/20260322_171209_music/normalization/attempt_1/discogs.csv",
    name="discogs",
)
lastfm = load_csv(
    "output/runs/20260322_171209_music/normalization/attempt_1/lastfm.csv",
    name="lastfm",
)
musicbrainz = load_csv(
    "output/runs/20260322_171209_music/normalization/attempt_1/musicbrainz.csv",
    name="musicbrainz",
)
...
scalar_string_columns = {
    "discogs": ["name", "artist", "release-country", "label", "genre"],
    "lastfm": ["name", "artist"],
    "musicbrainz": ["name", "artist", "release-country"],
}

for df_name, df in [("discogs", discogs), ("lastfm", lastfm), ("musicbrainz", musicbrainz)]:
    for col in scalar_string_columns.get(df_name, []):
...
if list_like_columns:
    discogs, lastfm, musicbrainz = normalize_list_like_columns(
        [discogs, lastfm, musicbrainz], list_like_columns
    )
...
if rb_correspondences_discogs_lastfm.empty:
    raise ValueError("Empty correspondences for expected pair: discogs_lastfm")
if rb_correspondences_discogs_musicbrainz.empty:
    raise ValueError("Empty correspondences for expected pair: discogs_musicbrainz")
if rb_correspondences_lastfm_musicbrainz.empty:
    raise ValueError("Empty correspondences for expected pair: lastfm_musicbrainz")
...
refined_correspondences_discogs_lastfm = clusterer.cluster(rb_correspondences_discogs_lastfm)
refined_correspondences_discogs_musicbrainz = clusterer.cluster(rb_correspondences_discogs_musicbrainz)
refined_correspondences_lastfm_musicbrainz = clusterer.cluster(rb_correspondences_lastfm_musicbrainz)

all_rb_correspondences = pd.concat(
    [
        refined_correspondences_discogs_lastfm,
        refined_correspondences_discogs_musicbrainz,
        refined_correspondences_lastfm_musicbrainz,
    ],
    ignore_index=True,
)
...
trust_map = {"discogs": 3.000, "musicbrainz": 1.000, "lastfm": 2.000}

strategy = DataFusionStrategy("fusion_strategy")

strategy.add_attribute_fuser("name", favour_sources, source_preferences=["musicbrainz", "discogs", "lastfm"])
...
strategy.add_attribute_fuser("tracks_track_name", favour_sources, source_preferences=["musicbrainz", "discogs", "lastfm"])
strategy.add_attribute_fuser("tracks_track_position", favour_sources, source_preferences=["musicbrainz", "discogs", "lastfm"])
strategy.add_attribute_fuser("tracks_track_duration", favour_sources, source_preferences=["musicbrainz", "lastfm", "discogs"])
