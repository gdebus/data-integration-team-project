...
def build_eval_view(fused_df, gold_df):
    ...
    for _, row in fused_df.iterrows():
        ...
        candidates = []
        if "eval_id" in row_dict and pd.notna(row_dict.get("eval_id")) and str(row_dict.get("eval_id")).strip():
            candidates = [str(row_dict.get("eval_id")).strip()]
...
def _drop_derived_fused_subcolumns(fused_df, gold_df):
    gold_cols = set(gold_df.columns)
    drop_cols = []
    for col in fused_df.columns:
        if col.startswith("_") or col in {"id", "eval_id"}:
            continue
        if col in gold_cols:
            continue
        for gold_col in gold_cols:
            if col.startswith(f"{gold_col}_") and gold_col not in fused_df.columns:
                drop_cols.append(col)
                break
    return fused_df.drop(columns=sorted(set(drop_cols)), errors="ignore")
...
EVAL_SET_PATH = "input/datasets/books/testsets/test_set.csv"
EVAL_DIR = os.path.join(OUTPUT_DIR, "pipeline_evaluation")
os.makedirs(EVAL_DIR, exist_ok=True)

# sealed_test => load provided sealed test set path
...
fusion_eval_set = load_csv(EVAL_SET_PATH, name="fusion_eval_set")
fused_eval = build_eval_view(fused, fusion_eval_set)

# ================================
# PRE-EVAL CLEANUP / ALIGNMENT
# ================================

fused_eval = _drop_derived_fused_subcolumns(fused_eval, fusion_eval_set)

# Use only columns meaningful in gold and present in both
gold_meaningful = _meaningful_gold_columns(fusion_eval_set)
shared_columns = sorted(
    c for c in (set(fused_eval.columns) & set(fusion_eval_set.columns) & gold_meaningful)
    if c not in {"id", "_id", "eval_id", "_eval_cluster_id", "_fusion_confidence"}
    and not c.startswith("_")
    and not c.endswith("_provenance")
)

# Numeric coercion before list detection/evaluation
numeric_hint_cols = [
    c for c in shared_columns
    if any(k in c.lower() for k in ["year", "count", "rating", "score", "price", "page", "num"])
]
fused_eval = _coerce_numeric_like(fused_eval, numeric_hint_cols)
fusion_eval_set = _coerce_numeric_like(fusion_eval_set, numeric_hint_cols)

# List-like normalization
list_eval_columns = detect_list_like_columns(
    [fused_eval[["eval_id"] + shared_columns] if "eval_id" in fused_eval.columns else fused_eval[shared_columns],
     fusion_eval_set[["id"] + shared_columns] if "id" in fusion_eval_set.columns else fusion_eval_set[shared_columns]],
...
if list_eval_columns:
    ...
    print("[LIST NORMALIZATION] columns=[]")
...
# Text cleanup for known textual fields
for col in shared_columns:
    if col in list_eval_columns:
        continue
    if fused_eval[col].dtype == "object" or str(fused_eval[col].dtype).startswith("string"):
        fused_eval[col] = fused_eval[col].apply(lambda x: x.strip() if isinstance(x, str) else x)
    if fusion_eval_set[col].dtype == "object" or str(fusion_eval_set[col].dtype).startswith("string"):
        fusion_eval_set[col] = fusion_eval_set[col].apply(lambda x: x.strip() if isinstance(x, str) else x)
