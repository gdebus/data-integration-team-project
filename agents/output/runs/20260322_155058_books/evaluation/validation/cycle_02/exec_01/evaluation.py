...
def build_eval_view(fused_df, gold_df):
    """
    Map fused cluster rows back to validation/test IDs using eval_id when present,
    otherwise using _fusion_sources. Use validation representation as target.
    """
...
# ================================
# 3. DROP PROVENANCE-ONLY COLUMNS / NON-1:1 DERIVED COLUMNS
# ================================
gold = gold[[c for c in gold.columns if not c.endswith("_provenance")]]

shared_columns = _meaningful_common_columns(fused_eval, gold)

# retain only directly comparable columns plus IDs / diagnostics
...
# force genres into list-style canonicalization for robust evaluation even if stored as strings
...
for col in shared_columns:
    ...
    if col in list_eval_columns:
        ...
        threshold = 0.85
        if col_l in {"title", "author", "publisher"}:
            threshold = 0.85
        elif col_l in {"language"}:
            threshold = 0.90
