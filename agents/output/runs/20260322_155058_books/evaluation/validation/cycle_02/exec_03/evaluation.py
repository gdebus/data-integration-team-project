...
try:
    ...
    if "__file__" in globals():
        ...
        _candidates.extend([_here, _here.parent, _here.parent.parent])
...
keep_fused_cols = [
    ...
    if c in shared_columns or c in {"eval_id", "_id", "_eval_cluster_id", "_fusion_confidence", "_fusion_sources"}
]
...
if list_eval_columns is None:
    list_eval_columns = []
else:
    list_eval_columns = list(list_eval_columns)
...
for col in shared_columns:
    ...
    if col in list_eval_columns:
        ...
        if "rating" in col_l or "score" in col_l:
...
def _coerce_numeric_cols(*dfs):
    numeric_hints = {
        "year", "count", "score", "rating", "page", "sales", "price",
