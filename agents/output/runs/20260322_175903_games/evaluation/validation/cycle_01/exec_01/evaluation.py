...
def _coerce_numeric_pairwise(df_list, columns):
    for df in df_list:
        for col in columns:
...
def _is_derived_subcolumn_without_gold_match(col, shared_cols):
    if "_" not in col or col.startswith("_"):
        return False
    parent = col.split("_", 1)[0]
    return parent in shared_cols and col not in shared_cols
...
# ================================
# 3. DROP NON-DIRECT / DERIVED MISMATCH COLUMNS
# ================================
shared_cols = set(fused_eval.columns) & set(fusion_test_set.columns)
drop_from_eval = {
    c for c in fused_eval.columns
    if _is_derived_subcolumn_without_gold_match(c, shared_cols)
}
if drop_from_eval:
    fused_eval = fused_eval.drop(columns=list(drop_from_eval), errors="ignore")
...
list_eval_columns = [
    ...
    and c in fusion_test_set.columns
    and _is_meaningful_gold_column(fusion_test_set, c)
]
...
if list_eval_columns:
    ...
    print(json.dumps({"list_normalization_columns": list_eval_columns}, indent=2))
...
numeric_hint_patterns = (
    "year", "score", "sales", "count", "price", "rating",
    "duration", "revenue", "latitude", "longitude", "pages"
)
numeric_eval_columns = [
    c for c in shared_columns
    if _is_meaningful_gold_column(fusion_test_set, c)
    and any(h in c.lower() for h in numeric_hint_patterns)
]
_coerce_numeric_pairwise([fused_eval, fusion_test_set], numeric_eval_columns)

if numeric_eval_columns:
    print(json.dumps({"numeric_coercion_columns": sorted(numeric_eval_columns)}, indent=2))
...
# ================================
# 8. CHOOSE EVALUATION COLUMNS
# ================================
candidate_columns = [
    c for c in sorted(shared_columns)
    if c not in {"id", "_id", "eval_id", "_eval_cluster_id", "_fusion_confidence", "_fusion_sources", "_fusion_source_datasets", "_fusion_metadata"}
...
for col in candidate_columns:
    ...
    elif col in {"id", "eval_id"} or col_lower.endswith("id") and col_lower not in {"userid"}:
        register_eval(col, exact_match, "exact_match")
    elif "bool" in col_lower or col_lower.startswith("is_") or col_lower.startswith("has_"):
        register_eval(col, boolean_match, "boolean_match")
    elif "date" in col_lower or "year" in col_lower or col == "releaseYear":
        register_eval(col, year_only_match, "year_only_match")
    elif any(k in col_lower for k in ["score", "sales", "rating", "price", "revenue", "count", "latitude", "longitude", "duration"]):
        tolerance = 0.5 if "score" in col_lower else 0.01
        register_eval(col, numeric_tolerance_match, "numeric_tolerance_match", tolerance=tolerance)
    elif any(k in col_lower for k in ["name", "title", "developer", "publisher", "platform", "series", "genre", "street", "city", "country", "label", "artist"]):
        threshold = 0.85 if col_lower in {"name", "developer", "publisher"} else 0.75
