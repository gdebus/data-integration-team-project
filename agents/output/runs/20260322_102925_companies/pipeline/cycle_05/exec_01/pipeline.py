...
refined_forbes_dbpedia = post_clusterer.cluster(corr_forbes_dbpedia)
refined_forbes_fullcontact = post_clusterer.cluster(corr_forbes_fullcontact)
refined_fullcontact_dbpedia = post_clusterer.cluster(corr_fullcontact_dbpedia)
...
trust_map = {"fullcontact": 3.000, "dbpedia": 2.000, "forbes": 1.000}

strategy = DataFusionStrategy("fusion_strategy")

strategy.add_attribute_fuser("name", voting)
strategy.add_attribute_fuser("country", prefer_higher_trust, trust_map={"forbes": 1.000, "dbpedia": 2.000, "fullcontact": 3.000})
...
if any("keypeople_name" in df.columns and df["keypeople_name"].apply(lambda x: isinstance(x, (list, tuple, set))).any() for df in datasets):
    from PyDI.fusion import union
    strategy.add_attribute_fuser("keypeople_name", most_complete)
else:
    strategy.add_attribute_fuser("keypeople_name", most_complete)
strategy.add_attribute_fuser("Sector", prefer_higher_trust, trust_map={"forbes": 1.000, "dbpedia": 2.000, "fullcontact": 3.000})
strategy.add_attribute_fuser("Continent", prefer_higher_trust, trust_map={"forbes": 1.000, "dbpedia": 2.000, "fullcontact": 3.000})
...
strategy.add_attribute_fuser("founded", prefer_higher_trust, trust_map=trust_map)
