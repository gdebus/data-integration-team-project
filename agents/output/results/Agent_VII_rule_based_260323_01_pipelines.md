...
amazon_small = load_csv(
    "output/runs/20260323_035936_books/normalization/attempt_1/amazon_small.csv",
    name="amazon_small",
)
goodreads_small = load_csv(
    "output/runs/20260323_035936_books/normalization/attempt_1/goodreads_small.csv",
    name="goodreads_small",
)
metabooks_small = load_csv(
    "output/runs/20260323_035936_books/normalization/attempt_1/metabooks_small.csv",
...
# Targeted inline normalization supported by visible mismatch patterns
for df in [amazon_small, goodreads_small, metabooks_small]:
    for col in [c for c in ["title", "author", "publisher", "language", "bookformat", "edition"] if c in df.columns]:
        df[col] = df[col].astype("string").str.strip()
    if "isbn_clean" in df.columns:
        df["isbn_clean"] = df["isbn_clean"].astype("string").str.strip()

# Goodreads author often contains suffixes like "(Goodreads Author)"
if "author" in goodreads_small.columns:
    goodreads_small["author"] = (
        goodreads_small["author"]
        .astype("string")
        .str.replace(r"\s*\(Goodreads Author\)\s*", "", regex=True)
        .str.strip()
    )
...
blocker_amazon_goodreads = EmbeddingBlocker(
    _flatten_list_cols_for_blocking(amazon_small, ["title", "author", "publish_year"]),
    _flatten_list_cols_for_blocking(goodreads_small, ["title", "author", "publish_year"]),
...
blocker_amazon_metabooks = EmbeddingBlocker(
    _flatten_list_cols_for_blocking(amazon_small, ["title", "author", "publisher"]),
    _flatten_list_cols_for_blocking(metabooks_small, ["title", "author", "publisher"]),
...
blocker_goodreads_metabooks = EmbeddingBlocker(
    _flatten_list_cols_for_blocking(goodreads_small, ["title", "author", "publisher"]),
    _flatten_list_cols_for_blocking(metabooks_small, ["title", "author", "publisher"]),
    text_cols=["title", "author", "publisher"],
    id_column="id",
    top_k=20,
...
if rb_correspondences_amazon_goodreads.empty:
    raise ValueError("Empty correspondences for expected pair amazon_small_goodreads_small")
if rb_correspondences_amazon_metabooks.empty:
    raise ValueError("Empty correspondences for expected pair amazon_small_metabooks_small")
if rb_correspondences_goodreads_metabooks.empty:
    raise ValueError("Empty correspondences for expected pair goodreads_small_metabooks_small")
...
# Goodreads/Metabooks enriched attributes
...
refined_correspondences_amazon_goodreads = clusterer.cluster(rb_correspondences_amazon_goodreads)
refined_correspondences_amazon_metabooks = clusterer.cluster(rb_correspondences_amazon_metabooks)
refined_correspondences_goodreads_metabooks = clusterer.cluster(rb_correspondences_goodreads_metabooks)

all_rb_correspondences = pd.concat(
    [
        refined_correspondences_amazon_goodreads,
        refined_correspondences_amazon_metabooks,
        refined_correspondences_goodreads_metabooks,
    ],
    ignore_index=True,
)
...
trust_map = {
    "amazon_small": 1.000,
    "goodreads_small": 2.000,
    "metabooks_small": 3.000,
}

strategy = DataFusionStrategy("fusion_strategy")

# Protected attributes — keep resolver unchanged
strategy.add_attribute_fuser("title", voting)
strategy.add_attribute_fuser("author", longest_string)
strategy.add_attribute_fuser("isbn_clean", prefer_higher_trust, trust_map=trust_map)
strategy.add_attribute_fuser("publish_year", prefer_higher_trust, trust_map=trust_map)
