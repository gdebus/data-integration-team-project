...
def _apply_spec_to_dataframe(
    ...
    # Remove ID and identifier columns from spec — these must never be normalized
    # as they serve as entity keys and may have leading zeros (e.g. ISBNs).
    _protected = set()
    if id_column:
        _protected.add(id_column)
    for col_name in list(spec_dict.keys()):
        col_lower = col_name.lower()
        if "id" in col_lower or "isbn" in col_lower or "ean" in col_lower or "upc" in col_lower:
            _protected.add(col_name)
    if _protected:
        spec_dict = {k: v for k, v in spec_dict.items() if k not in _protected}
